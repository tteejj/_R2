...
function New-BasePanel {
    ...
    $panel = @{
        ...
        AddChild = { 
            ...
            Invoke-WithErrorHandling -Component "$($self.Name).AddChild" -Context "AddChild" -ScriptBlock {
                if (-not $Child) {
            ...
            } -AdditionalData @{ Parent = $self.Name; ChildType = $Child.Type; ChildName = $Child.Name }
        }
        ...
        ClearChildren = {
            param($self)
            Invoke-WithErrorHandling -Component "$($self.Name).ClearChildren" -Context "Clearing all children" -ScriptBlock {
                foreach ($child in $self.Children) {
                    $child.Parent = $null
                }
                $self.Children = @()
                $self._isDirty = $true
            } -AdditionalData @{ Parent = $self.Name }
        }
        ...
        Show = { 
            ...
            } -AdditionalData @{ Panel = $self.Name }
        }
        ...
        HandleInput = { 
            ...
            Invoke-WithErrorHandling -Component "$($self.Name).HandleInput" -Context "Handling input" -ScriptBlock {
                ...
                return $false
            } -AdditionalData @{ Panel = $self.Name; Key = $Key }
        }
        
        GetContentBounds = {
            param($self)
            Invoke-WithErrorHandling -Component "$($self.Name).GetContentBounds" -Context "Getting content bounds" -ScriptBlock {
...
function global:New-TuiStackPanel {
    ...
    $panel.CalculateLayout = {
        ...
        Invoke-WithErrorHandling -Component "$($self.Name).CalculateLayout" -ScriptBlock {
            ...
            $self._isDirty = $false
            return $layout
        } -AdditionalData @{ Panel = $self.Name; Orientation = $self.Orientation }
    }
...
function global:New-TuiGridPanel {
    ...
    $panel._CalculateGridSizes = {
        ...
        Invoke-WithErrorHandling -Component "$($self.Name)._CalculateGridSizes" -ScriptBlock {
            ...
            return $sizes
        } -AdditionalData @{ Panel = $self.Name; Definitions = $definitions; TotalSize = $totalSize }
    }
    ...
    $panel.CalculateLayout = {
        ...
        Invoke-WithErrorHandling -Component "$($self.Name).CalculateLayout" -ScriptBlock {
            ...
            $self._isDirty = $false
            return $layout
        } -AdditionalData @{ Panel = $self.Name; RowDefs = $self.RowDefinitions; ColDefs = $self.ColumnDefinitions }
    }
